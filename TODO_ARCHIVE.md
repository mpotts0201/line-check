# TODO ARCHIVE — LineCheck (frozen 2026-07-28)

**This file is frozen.** It is the complete TODO history through 2026-07-27,
preserved verbatim when the active TODO.md was refreshed for the sync-trigger
rewrite (REFACTOR_PROPOSAL.md v3). Do not add or check off bullets here.

Worth rereading before interviews: the T7 Q&A weak spots (top of T7), the 7f
schema-drift story, 8b-ii.5's "the badge found the hole", and the poison-batch
known-limitation writeup under carry-over debt.
...
- **TODO_FOR_DUMMIES.md is the plain-words twin of this file.** This file stays dense (it's written as instructions to Claude Code); the twin says the same things in plain words for the human. Whenever a ticket here changes state or gets added, update the twin in the same commit.
...
- **8b** — History shows real per-audit sync state + the "Retry sync" fallback (moved from 7e). Split into one-per-session bullets below; they share the same per-audit surfacing. **MILESTONE: this is the send-out gate — see the T8 deadline note.**
  ...
  - [x] **8b-ii — Real badge on the History card.** Replace the hardcoded `<Text>Not synced</Text>` in `app/history/index.tsx` with the 8b-i state: "Synced ✓" / "Pending — N waiting" / "Not synced". This is the bullet that makes the whole T7 engine visible — until it lands, the demo can only *assert* that sync works. AC: complete an audit in airplane mode → card reads pending; reconnect → auto-sync drains → card reads Synced ✓ on next focus.
  - [x] **8b-ii.5 — Flush on completion (gap found live by 8b-ii's badge, 2026-07-22).** Landed with the offline guard: `requestFlush` does nothing when offline so no retry chances are spent in a dead zone (user's requirement — the alternative, a flush that doesn't count toward attempts, was rejected because it would let a poison row retry forever). Steady-online completion never syncs: auto-sync is edge-triggered (`autoSync.ts` flushes only on a false→true connectivity transition), so completing an audit while already online enqueues rows that sit until a manual Sync now or a connectivity blip. 7d was built to spec — the spec only covered the reconnect case; the badge made the hole visible the day it landed (the card honestly read "Pending — 13 waiting" and nothing reached Supabase until the manual tap). Fix: add `requestFlush()` to the auto-sync coordinator (reuse its existing single-flight guard: flush when idle, no-op while one is in flight), expose the instance from the app root, fire it after `completeAudit` succeeds (fire-and-forget — the audit is already durable in SQLite; this only makes the push prompt). AC: complete an audit while online → card reads Synced ✓ on next focus, no manual tap, no connectivity transition. **Verified by unit test:** requestFlush while idle flushes exactly once; requestFlush during an in-flight flush does not double-flush. Files: `src/sync/autoSync.ts` + its test, `app/_layout.tsx`, `app/audit/review/[auditId].tsx`.
...
- [ ] Run `npx eslint .` on Windows over commits `2f9d336` + `d963d56`, plus the 8b-i/8b-ii commits — ESLint can't execute in WSL2 (native binding is the Windows build), so Gate 0 is only half-verified on all of them. (8b-i's was confirmed green 2026-07-22.)
