...
- **8b** — History shows real per-audit sync state + the "Retry sync" fallback (moved from 7e). Split into one-per-session bullets below; they share the same per-audit surfacing. **MILESTONE: this is the send-out gate — see the T8 deadline note.**
  ...
  - [ ] **8b-ii.5 — Flush on completion (gap found live by 8b-ii's badge, 2026-07-22).** Steady-online completion never syncs: auto-sync is edge-triggered (`autoSync.ts` flushes only on a false→true connectivity transition), so completing an audit while already online enqueues rows that sit until a manual Sync now or a connectivity blip. 7d was built to spec — the spec only covered the reconnect case; the badge made the hole visible the day it landed (the card honestly read "Pending — 13 waiting" and nothing reached Supabase until the manual tap). Fix: add `requestFlush()` to the auto-sync coordinator (reuse its existing single-flight guard: flush when idle, no-op while one is in flight), expose the instance from the app root, fire it after `completeAudit` succeeds (fire-and-forget — the audit is already durable in SQLite; this only makes the push prompt). AC: complete an audit while online → card reads Synced ✓ on next focus, no manual tap, no connectivity transition. **Verified by unit test:** requestFlush while idle flushes exactly once; requestFlush during an in-flight flush does not double-flush. Files: `src/sync/autoSync.ts` + its test, `app/_layout.tsx`, `app/audit/review/[auditId].tsx`.
