...
- [ ] **8b** — History shows real "Synced ✓" badge backed by queue state. AC: badge flips only after Supabase rows confirmed. Also builds the **per-audit "Retry sync" fallback** (moved from 7e): shown only for a stuck audit (`attempts >= 3` and unsynced), manual retry resets `attempts`; this replaces the 7c global "Sync now" button. Both ride on the same per-audit sync-state surfacing. **MILESTONE: recruiter email goes out this day.**
